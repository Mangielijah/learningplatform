    ...
    if (isset($_POST['type'])&& $_POST['type']!=null) {
    	...
    	$question_content_id = $parent_id . $timestamp;
	     ...
	     if($type=="VIDEO") {
            ...
            if (empty($_FILES['video_file']['name'])||empty($_POST['description'])) {
            	...
            	header("Location:../lesson_upload.php?Error=Please Upload File(s) And Try Again");
		        exit();
            }
            else{
            $status = $_POST['status'];  
			...
			for ($i=0; $i < $filecount; $i++) { 
				...
				if (!$error === 0) {
		        	header("Location:../lesson_upload.php?Error=File Upload Error Please Try Again");
		            exit();
		        }
				elseif ($size > $maxSize) {
					header("Location:../lesson_upload.php?Error=File is to large");
		            exit();
				}
				elseif (! in_array($fileEX,$accepted)){
				   header("Location:../lesson_upload.php?Error=Unaccepted File accepted Files are MKV AVI MP$");
		           exit();
				}
				else{
		            $sql = "INSERT INTO lesson_content (lesson_id,content,status,timestamp_id) VALUES (?,?,?,?)";

					$stmt  = mysqli_stmt_init($conn);
					 if (!mysqli_stmt_prepare($stmt,$sql)) {
							header("Location:../lesson_upload.php?Error=File Upload Error");
							exit();
						}
						else{
             ...
             $sql = "INSERT INTO lessons (lesson_id,parent_id,lesson_type,lesson_description,status) VALUES (?,?,?,?,?)";
			...
			header("Location:../lesson_upload.php?succes=Files Uploaded Succesfully");
			exit();
            }
	    }
    	...
    	elseif ($type=="IMAGE") {
			...
			for ($i=0; $i < $filecount  ; $i++) { 
				...
				elseif (! in_array($fileEX,$accepted)){
				   header("Location:../lesson_upload.php?Error=Unaccepted File accepted Files are JPG JPEG WEBP PNG MPEG");
		           exit();
				}
				else{
		            $sql = "INSERT INTO lesson_content (lesson_id,content,status,timestamp_id) VALUES (?,?,?,?)";
	    ...
	    // RECEIVING AND STORING TEXT FILES
	      ...
	      if (empty($_POST['text'])||empty($_POST['description'])) {
		    ...
		    exit();
				...
				$sql = "INSERT INTO lessons(lesson_id,parent_id,lesson_type,lesson_description,status) VALUES (?,?,?,?,?)";
                $sql2 = "INSERT INTO lesson_content(lesson_id,content,status,timestamp_id) VALUES (?,?,?,?)";

				$stmt  = mysqli_stmt_init($conn);
				$stmt2  = mysqli_stmt_init($conn);
				if (!mysqli_stmt_prepare($stmt,$sql)||!mysqli_stmt_prepare($stmt2,$sql2)) {
					header("Location:../lesson_upload.php?Error=Text Upload Error");
					exit();
				}
				else{	
				mysqli_stmt_bind_param($stmt,"sssss",$timestamp,$parent_id,$type,$description,$status);
				...
				header("Location:../lesson_upload.php?succes=Uploaded Succesfully");
				exit();
	        }
	    }
	    ...
	    // RECIEVE AND STORE YOUTUBE LINKS
           ...
           if (empty($_POST['Youtube_link'])||empty($_POST['description'])) {
	    	...
	    	header("Location:../lesson_upload.php?Error=Error Choose A File Format And Continue");
	        exit();
	    }
    }
    else{
       header("Location:../lesson_upload.php?Error=Error Chose A File Format And Continue");
	   exit();
    }    
}	
else{
header("Location:../lesson_upload.php?Error=Error Please Try Again");
exit();
}
